...
void WRITE_Header8(hid_t*  file_id, 
  ...
  for (i = 0; i < *num_levels; i++) {
    ...
    prob_domain_3d[0].hi_i = (pow(2,i) * *t_nxb) - 1;
    prob_domain_3d[0].hi_j = (pow(2,i) * *t_nyb) - 1;
    prob_domain_3d[0].hi_k = (pow(2,i) * *t_nzb) - 1;
    ...
    prob_domain_2d[0].hi_i = (pow(2,i) * *t_nxb) - 1;
    prob_domain_2d[0].hi_j = (pow(2,i) * *t_nyb) - 1;
		   ...
		   int   iorder[])
{
  ...
  yl = 1.e30;
  zl = 1.e30;
  for (lb = 0; lb < *lnblocks_real; lb++) {
    if (bnd_box[lb* *mdim*2 + 2*0 + 0] < xl) {
      xl = bnd_box[lb* *mdim*2 + 2*0 + 0];
      /*      printf("Creating %d %d %3.2f %d \n",mpi_rank,*lnblocks, xl, lb);*/
    }
    if (bnd_box[lb* *mdim*2 + 2*1 + 0] < yl) {
      yl = bnd_box[lb* *mdim*2 + 2*1 + 0];
    }
  ...
  for (lb = 0; lb < *max_lnblocks; lb++) { 
    ...
    /*    printf("Trying to open level %i %d %d \n",lrefine[lb2]-1,mpi_rank,lb2); */
    sprintf(name, "/level_%i",lrefine[lb2]-1);
    if (lrefine[lb2] < 1) {
      sprintf(name, "/level_0");
    }
    ...
    if (*ndim == 3) {
      ...
      /*      printf("Creating boxes %3.2f %3.2f %3.2f \n",xmin, xleft,dx);*/
	...
	if (checkp_on_cc[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4];
		get_udim_chombo (*nvar, *nx, *ny, *nz, ivar, i, j, k, iorder, udim, it);
	...
	if (checkp_on_fc[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		get_udim_chombo (*nbndvar, *nx+1, *ny, *nz, ivar, i, j, k, iorder, udim, it);
		get_udim_chombo (*nbndvar, *nx+1, *ny, *nz, ivar, i+1, j, k, iorder, udim, it2);
	...
	if (checkp_on_fc[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4]; int it2[4];
		get_udim_chombo (*nbndvar, *nx, *ny+1, *nz, ivar, i, j, k, iorder, udim, it);
		get_udim_chombo (*nbndvar, *nx, *ny+1, *nz, ivar, i, j+1, k, iorder, udim, it2);
	...
	if (checkp_on_fc[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4]; int it2[4];
		get_udim_chombo (*nbndvar, *nx, *ny, *nz+k3d, ivar, i, j, k, iorder, udim, it);
		get_udim_chombo (*nbndvar, *nx, *ny, *nz+k3d, ivar, i, j, k+k3d, iorder, udim, it2);
	...
	if (checkp_on_ec[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4];
		get_udim_chombo (*nbndvare, *nx, *ny+1, *nz+k3d, ivar, i, j+jj, k+kk*k3d, iorder, udim, it);
	...
	if (checkp_on_ec[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4];
		get_udim_chombo (*nbndvare, *nx+1, *ny, *nz+k3d, ivar, i+ii, j, k+kk*k3d, iorder, udim, it);
	...
	if (checkp_on_ec[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4];
		get_udim_chombo (*nbndvare, *nx+1, *ny+1, *nz, ivar, i+ii, j+jj, k, iorder, udim, it);
	...
	if (checkp_on_nc[ivar] == 1) {
	  ...
	  for (k = kstart; k < kend; k++) {
		...
		int udim[4]; int it[4];
		get_udim_chombo (*nbndvarc, *nx+1, *ny+1, *nz+k3d, ivar, i+ii, j+jj, k+kk*k3d, iorder, udim, it);
...
#ifdef UNDERSCORE
void write_blocks_chombo_r8_ (char* file_name_in, 
#endif
#ifdef DOUBLE_UNDERSCORE
void write_blocks_chombo_r8__ (char* file_name_in, 
#endif
#ifndef UNDERSCORE 
#ifndef DOUBLE_UNDERSCORE
void write_blocks_chombo_r8 (char* file_name_in, 
#endif
#endif
			     float* time,
			     float* resolution,
			   ...
			   int   iorder[]) 
{
  ...
  CREATE_File8(file_name_in, &file_id);

  WRITE_Header8(&file_id, 
		time,
		resolution,
		num_components, 
		num_levels,
