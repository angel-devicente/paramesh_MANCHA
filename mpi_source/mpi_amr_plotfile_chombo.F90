...
!#define DEBUG
      ...
      subroutine amr_plotfile_chombo (file_num,time,resolution,t_nxb,t_nyb,t_nzb)
...
!-----------------------------!
      ...
      real, intent(in) :: time
      INTEGER, INTENT(IN) :: t_nxb,t_nyb,t_nzb
      ...
      integer :: ordering(2,3,4,1)
      integer :: iorder(4)

      CHARACTER(LEN=10),DIMENSION(10) :: mancha_var_names_background = [character(len=10) :: "te", "pe", "rho", "bx", "by", "bz", "e", "eint", "pelectron", "qtot"]
      CHARACTER(LEN=10),DIMENSION(13) :: mancha_var_names_perturbation = [character(len=10) :: "pelectron", "te", "pe", "rho", "bx", "by", "bz", "vx", "vy", "vz", "e", "eint", "qtot" ]
!---------------------------!
...
! COMPUTE TOTAL NO. OF BLOCKS STORED TO THE 'LEFT' OF THIS PROCESSOR
     ...
     &                             n_to_left_level(j-1,i)
      ...
      if (sum(n_to_left_level2(:)) .ne. n_to_left(mype)) then
         print *,' ERROR sum inconsistent (Warning: to look in the future [angelv] '
!         PRINT*, mype, "level2", n_to_left_level2
!         PRINT*, mype, "left", n_to_left
      end if
      ...
      do i=1,nvar
        if(checkp_on_cc(i)) then
           ...
           IF (file_num .EQ. 0) THEN
              compNames(icount) = mancha_var_names_background(icount2)
           ELSE
              compNames(icount) = mancha_var_names_perturbation(icount2)
           END IF
        end if
      enddo
